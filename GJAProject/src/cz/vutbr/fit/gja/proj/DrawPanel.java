...
public class DrawPanel extends JPanel implements Runnable
{
    ...
    /**Panel statistik - priznak povoleni*/
    protected boolean statsEnabled=false;
    /**Panel statistik*/
    protected DisplayStatsPanel statsPanel = new DisplayStatsPanel();
    /**Sirka a vyska panelu statistik*/
    final static int STATS_WIDTH=400,STATS_HEIGHT=400;
    ...
    /**
     * Nastavi priznak, ze je mozne zobrazit statisticky formular
     * @param enabled
     */
    ...
    protected void translatePoints(int x, int y)
    {
        ...
        if(isAncestorOf(statsPanel))
        {
            Point p = statsPanel.getLocation();
            p.x+=x;
            p.y+=y;
            statsPanel.setLocation(p);
        ...
        public void mouseClicked(MouseEvent evt)
        {
            ...
            if(!editEnable)
            {
                if(statsEnabled)
                {
                    ...
                    statsPanel.revalidate();
                    repaint();
                    fireStatusEvent("Statistiky zobrazeny", InfoEvent.CODE_INFO);
                }
                else
                {
                    remove(statsPanel);
                    fireStatusEvent("", InfoEvent.CODE_INFO);
                }
                return;
            }
